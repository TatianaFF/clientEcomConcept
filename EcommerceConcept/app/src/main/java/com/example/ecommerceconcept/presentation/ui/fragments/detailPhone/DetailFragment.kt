...
import com.example.ecommerceconcept.presentation.ui.adapters.AdapterDetails
import com.example.ecommerceconcept.presentation.ui.adapters.AdapterImagesPhone
    ...
    private fun init(){
        currentIdPhone?.let {
            viewModel.getPhoneById(currentIdPhone!!).observe(viewLifecycleOwner){ currentPhone ->
                ...
                viewModel.getCarts().observe(viewLifecycleOwner) { listCarts ->
                    listCarts.forEach {
                        Log.e("idPhone", it.idPhone)
                        Log.e("detail", currentIdPhone!!)
                        if (it.idPhone == currentIdPhone) btnAddToCart.visibility = View.INVISIBLE
                    }
